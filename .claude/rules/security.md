...
- Upload plików: walidacja MIME + rozszerzenia + rozmiaru, losowe nazwy plików, bucket S3 prywatny lub public-read tylko przez CDN. Nigdy nie ufaj `Content-Type` z requestu.
- Publiczne API: tylko odczyt w fazie 1; rate limiting (DRF throttling / warstwa AWS).
...
- Sekrety wyłącznie w zmiennych środowiskowych / AWS Secrets Manager — nic w repo, nic w logach, nic w komunikatach błędów zwracanych klientowi.
