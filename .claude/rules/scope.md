...
- Aplikacje Django podzielone domenowo (`blog`, `about`, docelowo `shop`, `bookings`, `accounts`) — nie mieszaj logiki bloga z resztą.
- Kod potrzebny w więcej niż jednej domenie (stałe typu `Language`/status publikacji, walidacja uploadu, sanityzacja HTML) trafia do neutralnej aplikacji `core`, a domenowe apps importują stamtąd — żadna z nich nie importuje bezpośrednio z drugiej. Duplikacja tego samego konceptu w dwóch appkach (np. osobny `Language` w `blog` i w `about`) tworzy dwa źródła prawdy do ręcznej synchronizacji przy każdej zmianie i jest defektem, nie akceptowalnym skrótem.
